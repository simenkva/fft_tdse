...
class MatrixSimulator(Simulator):
    ...
    def compute_eigenstates(self, n_states = None):
        ...
        H_op = LinearOperator((n,n), matvec=lambda x: gs.Apsi_vec(x,sigma=0), dtype=complex)
        ...
        E, U = eigsh(H_op, k=n_states, which='LM', sigma=sigma, return_eigenvectors=True)
        ...
        ic(np.linalg.norm(np.einsum('ki,kj->ij', self.U_mat.conj(), self.U_mat) - np.eye(n_states)))
    ...
    def time_step(self):
        ...
        psi_vec = expm_multiply(-1j * dt * self.charge * D * self.laser_pulse_fun(self.t+0.5*dt), psi_vec)
        psi_vec = expm_multiply(-0.5j * dt * H, psi_vec)
    ...
    def compute_psi(self):
        ...
        self.psi = np.einsum('ki,i->k', self.U_mat, self.psi_vec) / self.grid.dtau**.5
