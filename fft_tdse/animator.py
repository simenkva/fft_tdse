...
class AnimatorBase:
    ...
    def __init__(self, simulator, name="animation"):
        ...
        self.extension = ".png"
        self.format = (
            self.folder + self.basename + "%0" + str(self.digits) + "d" + self.extension
        )
        ic(self.format)
    ...
    def set_style(self, style):
        ...
        self.set_frame_postprocess(lambda anim: style.frame_postprocess(anim))
    ...
    def set_figwidth(self, fig_width):
        ...
        self.fig_height = (
            self.fig_height_pixels / self.fig_width_pixels * self.fig_width
        )
    ...
    def set_framesize(self, width_pixels, height_pixels):
        ...
        ic(
            self.fig_width_pixels,
            self.fig_height_pixels,
            self.fig_width,
            self.fig_height,
            self.dpi,
        )
    ...
    def init_figure(self):
        ...
        self.fig, self.ax = plt.subplots(
            1, 1, figsize=(self.fig_width, self.fig_height)
        )
...
class Animator1d(AnimatorBase):
    ...
    def __init__(self, simulator, name=None):
        ...
        self.xlim = [
            self.simulator.grid.a[0],
            self.simulator.b[0],
        ]  # None # set to [x0, x1] to zoom in on a region
        self.ylim = [-1.0, 1.0]
...
class Animator2d(AnimatorBase):
    ...
    def init_figure(self):
        ...
        self.image = self.ax.imshow(
            self.bmp, origin="lower", aspect="equal", extent=self.get_extent()
        )
    ...
    def add_potential_visualization_2d(
        self, transparent_range=[0, 0.1], cmap=dens_cmap, contour=False
    ):
        ...
        if self.simulator.dim != 2:
            raise ValueError(
                "add_potential_visualization_2d only works for 2d simulations"
            )

        V = self.simulator.ham.V
        # add potential visualization. use imshow, and make values below 0.1 transparent
        masked_V = np.ma.masked_where(
            (V < transparent_range[1]) & (V >= transparent_range[0]), V
        )
    ...
    def make_bmp(self, psi):
        ...
        if self.vis_type == "complex":
            bmp = phase_mag_vis2(
                phase_factor * psi_for_vis.T / scale,
                cmap=self.phase_cmap,
                mag_map=self.mag_map,
            )
        elif self.vis_type == "magnitude":
            bmp = mag_vis(
                psi_for_vis.T / scale, cmap=self.mag_cmap, mag_map=self.mag_map
            )
        else:
