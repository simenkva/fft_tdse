...
class FourierWavefunction:
    ...
    def density(self, n):
        ...
        notn = lambda n: tuple(
            k for k in range(self.grid.d) if k != n
        )  # all axes except n
        rho = (
            np.sum(np.abs(self.psi) ** 2, axis=notn(n))
            * self.grid.dtau
            / self.grid.h[n]
        )
        return rho
    ...
    def current(self, n):
        ...
        jp = (
            np.sum(
                np.imag(
                    ifftn(1j * self.grid.kk[n] * self.phi, norm="ortho")
                    * self.psi.conj()
                ),
                axis=notn(n),
            )
    ...
    def phiNorm(self):
        return np.linalg.norm(self.phi) * self.grid.dtau**0.5

    def psiNorm(self):
        return np.linalg.norm(self.psi) * self.grid.dtau**0.5
    ...
    def normalizePhi(self):
        ...
        n = np.linalg.norm(self.phi) * self.grid.dtau**0.5
...
class FourierHamiltonian:
    ...
    def energy(self, wf, Efield=0.0):
        ...
        return self.grid.dtau * (
            np.sum(wf.phi.conj() * self.T * wf.phi)
            + np.sum(wf.psi.conj() * (self.V - Efield * self.D) * wf.psi)
        )
    ...
    def angmoms(self, wf):
        ...
        kx = self.grid.kk[0]
        ky = self.grid.kk[1]
        Lz_psi = x * ifftn(ky * wf.phi, norm="ortho") - y * ifftn(
            kx * wf.phi, norm="ortho"
        )
...
class Propagator:
    ...
    def strang(self, wf, t, will_do_another_step=True):
        ...
        if self.time_dependent:
            ...
            wf.applySpaceOperator(self.Eprop(t + 0.5 * self.dt))
        else:
...
class GroundStateComputer:
    ...
    def Apsi_vec(self, psi_vec, sigma=0.0):
        ...
        return (
            self.ham.apply(psi_vec.reshape(ng)).reshape((np.prod(ng),))
            - sigma * psi_vec
        )
    ...
    def invit(self, tol=1e-9, cgtol=1e-9, maxit=400, sigma=0):
        ...
        A = LinearOperator(
            (n, n), matvec=lambda x: self.Apsi_vec(x, sigma=sigma), dtype=complex
        )
        ...
        assert (
            converged
        ), f"Inverse iterations failed to converge in {maxit} iterations!"
        self.wf.setPsi(psi.reshape(shape), set_dual=True, normalize=True)
        self.E = E.real

        return self.E

    def imagTimePropagation(self, tol=1e-9, dt=0.01, maxit=400, sigma=0):
        ...
        for k in range(maxit):
            ...
            # self.wf.setPsi(psi, set_dual = False)
            delta = np.linalg.norm(self.wf.psi - psi_prev) * self.grid.dtau**0.5
