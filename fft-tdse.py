...
class FourierGrid:
    def __init__(self,L,ng):
        ...
        L = (2,3,4)
        ng = (128,128,128)
        grid = FourierGrid(L,ng)

        L = tuple of box widths for each dimension
        ng = tuple of number of grid points for each dimension. should be powers of 2.

        """
        self.defineGrid(L,ng)
        return
    ...
    def defineGrid(self,L,ng):
        ...
        self.k = []
        for i in range(d):
            x0,k0 = self.fftgrid(-L[i], L[i], ng[i])
            self.x.append(x0)
            self.k.append(k0)
    ...
    def __str__(self):
        return str(self.__class__) + f": L = {self.L}, ng = {self.ng}"  #self.__dict__
...
class GroundStateComputer:
    ...
    def invit(self,tol=1e-9,cgtol=1e-9, maxit=400,sigma=0):
        ...
        for k in range(maxit):
            ...
            Hpsi = ham.apply(psi.reshape(shape)).reshape((n,))
            E = np.inner(psi,Hpsi)
